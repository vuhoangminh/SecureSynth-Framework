...
class MLPDiffusion(nn.Module):
    def __init__(self, d_in, dim_t=512, num_blocks=3):
        super().__init__()
        self.dim_t = dim_t
        ...
        # # Added by Minh
        # mlp_layers = [nn.Linear(dim_t, dim_t * 2), nn.SiLU()]

        # for _ in range(num_blocks - 2):
        #     mlp_layers.extend([nn.Linear(dim_t * 2, dim_t * 2), nn.SiLU()])

        # mlp_layers.extend(
        #     [nn.Linear(dim_t * 2, dim_t), nn.SiLU(), nn.Linear(dim_t, d_in)]
        # )
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.loss_version == 0:
            ...
            loss, loss_corr, loss_dwp = self.loss_fn(self.denoise_fn_D, x)
            return loss.mean(-1).mean() + loss_corr + loss_dwp
