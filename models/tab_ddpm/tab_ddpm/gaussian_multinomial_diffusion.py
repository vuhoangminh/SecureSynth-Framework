...
### Added by Minh
from engine.custom_loss import CorrectedCorrelationLoss, DistributionLoss
...
class GaussianMultinomialDiffusion(torch.nn.Module):
    def __init__(
        ...
        is_loss_corr_num=0,
        is_loss_dwp_num=0,
        n_moment_loss_dwp_num=4,
        empirical_class_dist=None,
        num_samples=0,
        train_data_tensor=None,  # dataset
        ### Added by Minh
    ):
        ...
        self.is_loss_num = is_loss_num
        self.is_loss_corr_num = is_loss_corr_num
        self.is_loss_dwp_num = is_loss_dwp_num
        self.n_moment_loss_dwp_num = n_moment_loss_dwp_num
        self.empirical_class_dist = empirical_class_dist
        self.num_samples = num_samples
        self.train_data_tensor = train_data_tensor  # dataset
        ### Added by Minh
    ...
    def log_sample_categorical(self, logits):
        ...
        for i in range(len(self.num_classes)):
            ...
            ## Added
            uniform = torch.rand_like(one_class_logits, requires_grad=True)
            ## Added
    ...
    def log_sample_categorical_during_training(self, logits, differentiable=True):
        full_sample = []

        for i in range(len(self.num_classes)):
            one_class_logits = logits[:, self.slices_for_classes[i]]

            # Add Gumbel noise to the logits for sampling
            uniform = torch.rand_like(one_class_logits)
            gumbel_noise = -torch.log(-torch.log(uniform + 1e-30) + 1e-30)

            if differentiable:
                # Use Gumbel-Softmax to sample in a differentiable manner
                tau = 1.0  # Temperature for Gumbel-Softmax, adjust as needed
                gumbel_softmax_sample = torch.nn.functional.gumbel_softmax(
                    one_class_logits + gumbel_noise, tau=tau, hard=False, dim=1
                )
                full_sample.append(gumbel_softmax_sample)
            else:
                # Use non-differentiable argmax for sampling
                sample = (gumbel_noise + one_class_logits).argmax(dim=1)
                full_sample.append(sample.unsqueeze(1))

        if differentiable:
            # Concatenate the soft samples
            full_sample = torch.cat(full_sample, dim=1)
            log_sample = (
                full_sample  # Direct soft samples without conversion to one-hot
            )
        else:
            # Convert categorical samples into log one-hot encoding (non-differentiable)
            full_sample = torch.cat(full_sample, dim=1)
            log_sample = index_to_log_onehot(full_sample, self.num_classes)
    ...
    def _mixed_loss(self, x, out_dict):
        ...
        model_out_num = model_out[:, : self.num_numerical_features]  # epsilon
        model_out_cat = model_out[:, self.num_numerical_features :]  # l_cat1, l_cat2
        ...
        ### Added by Minh
        model_out_num_pred = model_out_cat_pred = None
        if x_num.shape[1] > 0:
            model_out_num_pred = model_out_num

            if self.is_loss_num > 0:
                loss_dwp_num = self.is_loss_dwp_num * DistributionLoss()(
                    noise, model_out_num_pred, alpha=1, n=self.n_moment_loss_dwp_num
                )
                loss_corr_num = self.is_loss_corr_num * CorrectedCorrelationLoss()(
                    noise, model_out_num_pred
                )
            else:
                loss_dwp_num = loss_corr_num = 0

        else:
            loss_dwp_num = loss_corr_num = 0

        if x_cat.shape[1] > 0:
            model_log_prob = self.predict_start(
                model_out_cat, log_x_t=log_x_cat_t, t=None, out_dict=None
            )
            model_out_cat_pred = torch.exp(model_log_prob)
            ...
            loss_corr = self.is_loss_corr * CorrectedCorrelationLoss()(
                torch.exp(log_x_cat), model_out_cat_pred
            )
        else:
            loss_dwp = loss_corr = 0
    ...
    def _mixed_loss_sample_during_train(self, x, out_dict):
        b = x.shape[0]
        device = x.device
        t, pt = self.sample_time(b, device, "uniform")
        ...
        # TODO: add auxilary loss here
        ### Added by Minh
        x_gen, y_gen = self.sample_during_training(b)
        x_sample = torch.cat((x_gen, torch.unsqueeze(y_gen["y"], 1)), dim=1)
    ...
    def mixed_loss_concat(self, x, out_dict):
        b = x.shape[0]
        device = x.device
        t, pt = self.sample_time(b, device, "uniform")
        ...
        if x_num.shape[1] > 0:
            model_out_num_pred = self.gaussian_p_sample(
                model_out_num, x_num_t, t, clip_denoised=False
            )["sample"]

        if x_cat.shape[1] > 0:
            model_log_prob = self.p_sample_during_training(
                model_out_cat, log_x=log_x_cat_t, t=t, out_dict=out_dict
            )
            model_out_cat_pred = model_log_prob

        if model_out_num_pred is None:
            x_sample = model_out_cat_pred
        elif model_out_cat_pred is None:
            x_sample = model_out_num_pred
        elif model_out_num_pred is None and model_out_cat_pred is None:
            raise ValueError
        else:
            x_sample = torch.cat([model_out_num_pred, model_out_cat_pred], dim=1)

        if x_num.shape[1] == 0:
            x_input = torch.exp(log_x_cat_t)
        elif x_cat.shape[1] == 0:
            x_input = x_num_t
        else:
            x_input = torch.cat([x_num_t, torch.exp(log_x_cat_t)], dim=1)

        x_input = x_input.float()
        x_sample = x_sample.float()
        loss_dwp = self.is_loss_dwp * DistributionLoss()(
            x_input, x_sample, alpha=1, n=self.n_moment_loss_dwp
        )
        loss_corr = self.is_loss_corr * CorrectedCorrelationLoss()(x_input, x_sample)
        loss_dwp_num = loss_corr_num = 0
        ### Added by Minh
    ...
    def mixed_loss_separate_loss_num_to_zero(self, x, out_dict):
        # def mixed_loss(self, x, out_dict):
        """
        running mixed_loss_separate and set loss_dwp_num = loss_corr_num = 0
        Result: works well
        """
        b = x.shape[0]
        device = x.device
        ...
        ### Added by Minh
        model_out_num_pred = model_out_cat_pred = None
        loss_dwp = loss_corr = loss_dwp_num = loss_corr_num = 0

        if x_num.shape[1] > 0:
            noise_1d = torch.flatten(noise)
            model_out_num_1d = torch.flatten(model_out_num)
            loss_dwp_num = (
                self.is_loss_dwp_num
                * DistributionLoss()(
                    noise_1d, model_out_num_1d, alpha=1, n=self.n_moment_loss_dwp_num
                )
                if self.is_loss_dwp_num > 0
                else 0
            )
            loss_corr_num = 0
        else:
        ...
        if x_cat.shape[1] > 0:
            ...
            loss_dwp = (
                self.is_loss_dwp
                * DistributionLoss()(
                    torch.exp(log_x_cat_t),
                    model_out_cat_pred,
                    alpha=1,
                    n=self.n_moment_loss_dwp,
                )
                if self.is_loss_dwp > 0
                else 0
            )

            loss_corr = (
                self.is_loss_corr
                * CorrectedCorrelationLoss()(torch.exp(log_x_cat_t), model_out_cat_pred)
                if self.is_loss_corr > 0
                else 0
            )
        else:
            loss_dwp = loss_corr = 0
    ...
    def mixed_loss(self, x, out_dict):
        # def mixed_loss_separate_loss(self, x, out_dict):
        """
        running mixed_loss_separate and set loss_dwp_num = loss_corr_num = 0
        Result: works well
        """
    ...
    ## Added
    def p_sample_during_training(self, model_out, log_x, t, out_dict):
        model_log_prob = self.p_pred(model_out, log_x=log_x, t=t, out_dict=out_dict)
        out = self.log_sample_categorical_during_training(model_log_prob)
        return out

    def sample_during_training(self, num_samples):
        y_dist = self.empirical_class_dist.float()
        b = num_samples
        device = self.log_alpha.device
        z_norm = torch.randn((b, self.num_numerical_features), device=device)
        ...
        for i in reversed(range(0, self.num_timesteps)):
            if i > 0:  # Context manager
                ...
                if has_cat:
                    log_z = self.p_sample_during_training(
...
The issue arises because argmax is a non-differentiable operation. In PyTorch, any non-differentiable operation like argmax effectively "cuts" the computational graph, meaning that the resulting tensor sample will not have a grad_fn and won't allow gradient flow.

To allow gradients to propagate through this operation, you'll need to avoid argmax and use a differentiable alternative. Common techniques to achieve this in a way that preserves gradient flow include using soft approximations, such as:

Gumbel-Softmax (Softmax with Gumbel Noise): This allows sampling from a categorical distribution in a differentiable way by applying the softmax function after adding Gumbel noise.

Softmax Instead of Argmax: You can use the softmax function to represent a "softer" version of the argmax operation that still allows gradients to flow.

Example Using Gumbel-Softmax
Here's how you can modify your code to use a differentiable approximation:


import torch
import torch.nn.functional as F

# Assume gumbel_noise and one_class_logits already have grad_fn and are differentiable
gumbel_noise = torch.randn((10, 5), requires_grad=True)
one_class_logits = torch.randn((10, 5), requires_grad=True)

# Compute softmax over logits + gumbel noise
temperature = 1.0  # You can adjust this temperature for "softness"
sample = F.gumbel_softmax(gumbel_noise + one_class_logits, tau=temperature, hard=False)

# Now 'sample' has grad_fn and allows backpropagation

@TODO
Try sample at the end of each batch
Compute distribution from the beginning
Compare
"""
