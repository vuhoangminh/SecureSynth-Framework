        ...
        for i_epoch in range(epochs):
            ...
            for id_ in range(steps_per_epoch):
                ...
                if (
                    ...
                    # Added by Minh -- only dis. loss
                    and self.args.loss_version != 5
                    # Added by Minh -- only dis. loss
                ):
                    if self.args.loss_version == 1:
                        loss_corr = self.is_loss_corr * CorrelationLoss()(
                            real, fakeact
                        )  # version 1
                    else:
                ...
                if (
                    ...
                    # Added by Minh -- only cor. loss
                    and self.args.loss_version != 4
                    # Added by Minh -- only cor. loss
                ):
                    if self.args.loss_version == 1:
                        loss_dwp = self.is_loss_dwp * DWPLoss()(
                            train_data_tensor, fakeact
                        )  # version 1
                    elif self.args.loss_version == 3:
                        loss_dwp = self.is_loss_dwp * NormalizedDistributionLoss()(
                            train_data_tensor,
                            fakeact,
                            alpha=1,
