...
def get_df_score_comparison_gan(
    ...
    def is_enough_data(dataset, model, loss_version, condvec, dir, N=20):
        ...
        for method, _, _ in methods_info:
            l = len(
                hyperopt_utils.load_project(
                    f"{dir}/{method}/{dataset}_{model}_loss_version-{loss_version}-{condvec}.hyperopt",
                    is_print=False,
                ).trials
            )
            trials_lengths.append(l)
    ...
    for dataset in datasets:
        ...
        for model in models:
            ...
            for loss_version in [0, 2]:
                for condvec in [0, 1]:
                    df_temp = get_row(dataset, model, loss_version, condvec, dir)
                    if df_temp is not None:
                        if df_score is None:
                            df_score = (
                                df_temp.copy()
...
def main_bo_ml():
    ...
    path_df_score = "database/results/bo_compare_ml.csv"
    ...
    for loc1, loc2 in [(0, 1), (0, 2)]:
        eval_pair(df_score, loc1, loc2)

    for loc1, loc2 in [(1, 0), (1, 2)]:
        eval_pair(df_score, loc1, loc2)

    for loc1, loc2 in [(2, 0), (2, 1)]:
        eval_pair(df_score, loc1, loc2)
...
def main_bo_gan():
    ...
    path_df_score = "database/results/bo_compare_gan.csv"
