...
def init_search_space(args):
    ...
    if args.model_type == "mlp":
        min_n_layers, max_n_layers, d_min, d_max = 2, 2, 8, 8
        ...
        min_n_layers, max_n_layers = 3, 3
        search_space["n_blocks"] = hp.choice(
            "n_blocks", list(range(min_n_layers, max_n_layers + 1, 1))
        )
        search_space["d_main"] = hp.choice("d_main", list(range(256, 256 + 1, 1)))
        search_space["d_hidden"] = hp.choice("d_hidden", list(range(256, 256 + 1, 1)))
        search_space["dropout_first"] = hp.choice("dropout_first", [0.2])
        search_space["dropout_second"] = hp.choice("dropout_second", [0.2])
    ...
    search_space["lr"] = hp.choice("lr", [0.0003])
    search_space["weight_decay"] = hp.choice("weight_decay", [0.0])
    search_space["gaussian_loss_type"] = hp.choice("gaussian_loss_type", ["mse"])
    search_space["batch_size"] = hp.choice("batch_size", [128])
    search_space["num_timesteps"] = hp.choice("num_timesteps", [1000])

    if args.is_test:
        search_space["steps"] = hp.choice("steps", [1000])
    else:
        search_space["steps"] = hp.choice("steps", [10000])
        ...
        search_space.update(
            {
                ...
                "n_moment_loss_dwp": hp.choice("n_moment_loss_dwp", [2]),
                "is_loss_num": hp.choice("is_loss_num", [1]),
                "is_loss_corr_num": hp.uniform("is_loss_corr_num", -2, 4),
                "is_loss_dwp_num": hp.uniform("is_loss_dwp_num", -40, -6),
                "n_moment_loss_dwp_num": hp.choice("n_moment_loss_dwp_num", [2]),
            }
        )
...
def objective(params):
    ...
    return construct_return_dict(loss, reason, params, None)
