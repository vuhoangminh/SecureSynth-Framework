...
parser.add_argument(
    ...
    # choices=["adult", "covertype", "credit", "intrusion"],
    type=str,
)
parser.add_argument(
    "--epochs",
...
def main():
    ...
    if os.path.exists(
        os.path.join(args.dir_logs, "df_score_data_sufficient.csv")
    ) or os.path.exists(os.path.join(args.dir_logs, "synthetic_full.csv")):
        print(
            "Process stopped: The output file 'df_score_data_sufficient.csv' already exists."
        )
        sys.exit(0)  # Use a 0 exit code for successful termination
