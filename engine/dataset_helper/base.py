...
# Default TabDDPM config — hardcoded so there is no dependency on database/prepared/config.toml.
# Values mirror database/dataset/config.toml exactly. None encodes TOML "__none__" (lib handles
# the round-trip). Both _prep_tabddpm_config_toml_mlp and _prep_tabddpm_config_toml_resnet
# deepcopy this dict and patch only the dataset-specific keys.
_TABDDPM_BASE_CONFIG = {
...
class EvaluatedDataset(abc.ABC):
    ...
    def _prep_tabddpm_config_toml_mlp(self):
        base_config = copy.deepcopy(_TABDDPM_BASE_CONFIG)
