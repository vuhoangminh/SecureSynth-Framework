...
def _compute_rdp_tree_restart(sigma, steps_list, alpha):
    ...
    if np.isinf(alpha):
        return np.inf
    tree_depths = [
        math.floor(math.log2(float(steps))) + 1 for steps in steps_list if steps > 0
    ]
...
def compute_rdp_sample_without_replacement(q, noise_multiplier, steps, orders):
    ...
    if np.isscalar(orders):
        ...
        rdp = np.array(
            [
                _compute_rdp_sample_without_replacement_scalar(
                    q, noise_multiplier, order
                )
                for order in orders
            ]
        )
...
def compute_heterogeneous_rdp(
    sampling_probabilities, noise_multipliers, steps_list, orders
):
    ...
    rdp = 0
    for q, noise_multiplier, steps in zip(
        sampling_probabilities, noise_multipliers, steps_list
    ):
